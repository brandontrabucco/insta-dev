...
export SERVER_DIR=${SERVER_DIR:-"insta/javascript/server"}
export SERVER_SCRIPT=${SERVER_SCRIPT:-"insta/javascript/server/src/index.js"}
...
seq 1 ${MAX_ERRORS} | xargs --process-slot-var WORKER_IDX -I {} -P ${SERVER_WORKERS} \
    bash -c "node ${SERVER_SCRIPT} \\\$((WORKER_IDX + ${SERVER_BASE_PORT})) >> playwright.log 2>&1"
